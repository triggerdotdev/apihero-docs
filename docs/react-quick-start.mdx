...
async function initProxy() {
  const projectKey = "INSERT_PROJECT_KEY";
  if (typeof window !== "undefined") {
    ...
    //this will proxy requests from the server
...
If you are using Next.js and want to proxy server requests, you need to check that your tsconfig.json file has `"moduleResolution": "nodenext"` in the `compilerOptions` section. It will not work with `"node"`. See the [Node docs](/node-quick-start#2-check-your-tsconfigjson-configuration-if-youre-using-typescript) for more information.

:::

:::tip

Although it may seem unsafe, it is okay to expose the `projectKey` in your client. The `projectKey` is a unique identifier for your project, and is used to authenticate with the API Hero API. It is not a secret, think of it more as a username.
