...
The `createQuery` function from `@apihero/react` will create an "endpoint hook", in this case for the [`repos.getRepo`](https://docs.github.com/en/rest/repos/repos#get-a-repository) endpoint.

```ts title="src/endpoints.ts"
import { repos } from "@apihero/github";
import { createQuery } from "@apihero/react";

export const useGetRepository = createQuery(repos.getRepo);
```
